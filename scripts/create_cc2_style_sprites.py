...
class CC2StyleSpriteGenerator:
    ...
    def create_infantry_sprite(self, faction: str, size: int = 32) -> Image.Image:
        ...
        draw.ellipse(
            [
                center_x - helmet_radius,
                center_y - helmet_radius - 2,
                center_x + helmet_radius,
                center_y + helmet_radius - 2,
            ],
            fill=uniform_dark,
        ...
        body_height = size // 3
        draw.ellipse(
            [center_x - body_width, center_y, center_x + body_width, center_y + body_height],
            fill=uniform_mid,
        )
        ...
        pack_size = size // 8
        draw.rectangle(
            [center_x - pack_size, center_y + 2, center_x + pack_size, center_y + pack_size + 2],
        ...
        draw.line(
            ...
            fill=self.colors["metal_dark"],
            width=2,
        )

        # 高光
        draw.ellipse(
            [center_x - helmet_radius // 2, center_y - helmet_radius - 1, center_x, center_y - 1],
            fill=uniform_light,
        )
    ...
    def create_tank_sprite(self, faction: str, size: int = 48) -> Image.Image:
        ...
        hull_rect = [
            center_x - hull_width // 2,
            center_y - hull_height // 2,
            center_x + hull_width // 2,
            center_y + hull_height // 2,
        ]
        ...
        draw.ellipse(
            [
                center_x - turret_radius,
                center_y - turret_radius,
                center_x + turret_radius,
                center_y + turret_radius,
            ],
            fill=self.colors["tank_dark"],
        ...
        draw.rectangle(
            [
                center_x - barrel_width // 2,
                center_y - turret_radius - barrel_length,
                center_x + barrel_width // 2,
                center_y - turret_radius,
            ],
            fill=self.colors["metal_mid"],
            outline=self.colors["metal_dark"],
        )
        ...
        draw.rectangle(
            [
                center_x - track_offset,
                center_y - hull_height // 2 - 2,
                center_x - track_offset + track_width,
                center_y + hull_height // 2 + 2,
            ],
        ...
        # 高光
        draw.ellipse(
            [center_x - turret_radius // 2, center_y - turret_radius // 2, center_x, center_y],
    ...
    def create_vehicle_sprite(self, vehicle_type: str, size: int = 40) -> Image.Image:
        ...
        if vehicle_type == "jeep":
            ...
            draw.rectangle(
                ...
                fill=self.colors["allied_mid"],
                outline=self.colors["allied_dark"],
            )
            ...
            for wx, wy in wheel_positions:
                draw.ellipse(
                    [wx - wheel_radius, wy - wheel_radius, wx + wheel_radius, wy + wheel_radius],
                    fill=self.colors["black"],
                )
            ...
            draw.rectangle(
                ...
                fill=self.colors["tank_mid"],
                outline=self.colors["tank_dark"],
            )
            ...
            draw.ellipse(
                [
                    ...
                    center_y - body_height // 2 - wheel_radius - 2,
                    center_x + wheel_radius,
                    center_y - body_height // 2 + wheel_radius - 2,
                ],
                fill=self.colors["black"],
    ...
    def create_building_sprite(self, building_type: str, size: int = 64) -> Image.Image:
        ...
        if building_type == "house":
            ...
            draw.rectangle(
                [size // 3, size // 2, size // 3 + window_size, size // 2 + window_size],
                fill=window_color,
            )
            draw.rectangle(
                [size * 2 // 3 - window_size, size // 2, size * 2 // 3, size // 2 + window_size],
                fill=window_color,
            )
    ...
    def create_tree_sprite(self, size: int = 48) -> Image.Image:
        ...
        for color, radius, y_offset in zip(foliage_colors, radii, y_offsets, strict=False):
            draw.ellipse(
                [
                    center_x - radius,
    ...
    def generate_all_sprites(self):
        ...
        # 生成盟军步兵
        print("  生成盟军步兵...")
        for _i, unit_type in enumerate(["rifleman", "mg_team", "officer", "engineer"]):
