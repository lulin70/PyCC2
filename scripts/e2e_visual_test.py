        ...
        for mode in ["fast_move", "sneak", "defend", "normal"]:
            try:
                ...
                renderer._draw_movement_mode_overlay(
                    unit=unit, cx=50, cy=50, radius=15, base_color=(0, 255, 80)
                )
...
class TestD2_LightingSystem:
    ...
    def test_7_lighting_config_dataclass(self):
        ...
        assert config.enable_dynamic_lights
    ...
    def test_8_time_of_day_tint_colors(self):
        ...
        print("✅ Time-of-day tints: dawn/noon/dusk/night all work")
    ...
    def test_9_dynamic_light_lifecycle(self):
        ...
        renderer.spawn_dynamic_light(
            position=(100, 200), radius=50, intensity=0.8, color=(255, 200, 100), duration_ms=500
        )
        ...
        assert len(renderer._dynamic_lights) == initial_count + 1, (
            "Light should still exist at 250ms"
        )

        # Update past duration (should expire)
        renderer.update_dynamic_lights(dt_ms=300)
        assert len(renderer._dynamic_lights) == initial_count, (
            "Light should have expired after 550ms total"
        )
...
class TestD3_ParticleEffects:
    ...
    def test_13_explosion_ring_generation(self):
        ...
        print(
            f"✅ Explosion ring spawned: max_radius={particle['max_radius']}px, duration={particle['duration']}ms"
        )
    ...
    def test_14_smoke_cloud_properties(self):
        ...
        print(
            f"✅ Smoke cloud: drift=({particle['drift_x']:.2f}, {particle['drift_y']:.2f}), duration={particle['duration']}ms"
        )
    ...
    def test_16_hit_marker_damage_types(self):
        ...
        print(
            f"✅ Hit markers: {len(colors_seen)} distinct colors for {len(damage_types)} damage types"
        )
    ...
    def test_18_blood_pool_persistence(self):
        ...
        is_persistent = (
            particle.get("duration", 99999) == 0
            or particle.get("persistent", False)
            or particle.get("duration", 0) > 5000
        )
        assert is_persistent, (
            "Blood pool should persist (duration=0, persistent=True, or very long duration)"
        )
    ...
    def test_20_top_down_particle_constraints(self):
        ...
        assert (
            "gravity" not in source_lower
            or "no gravity" in source_lower
            or "friction" in source_lower
