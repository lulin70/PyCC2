...
from pycc2.domain.components.weapon_component import WeaponComponent, WeaponType

def make_unit(id='t', name='Test', unit_type=UnitType.INFANTRY_SQUAD, faction=Faction.ALLIES):
    return Unit(id=id, name=name, unit_type=unit_type, faction=faction,
        health=HealthComponent(hp=100, max_hp=100), morale=MoraleComponent(value=80),
        weapon=WeaponComponent(primary_weapon_id='rifle', ammo_remaining=120, max_ammo=120),
        position=PositionComponent(0,0), vision=VisionComponent(10))
...
assert result == False
print(f'[OK] Building interior auto-switch function')

# 11. Campaign UI
from pycc2.presentation.ui.campaign_ui import CampaignUI
print(f'[OK] Campaign UI')
