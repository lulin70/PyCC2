...
def run_fps_benchmark() -> Path:
    ...
    if result.returncode != 0:
        print(
            f"[update_perf_baseline] WARN: pytest exited {result.returncode}, "
    ...
    if not candidates:
        raise FileNotFoundError(
            f"No benchmark JSON found in {DEFAULT_BENCHMARK_DIR}. Run pytest-benchmark first."
        )
    latest = candidates[-1]
...
def git_commit_baseline() -> bool:
    ...
    result = subprocess.run(["git", "commit", "-m", commit_msg], cwd=REPO_ROOT, check=False)
    if result.returncode == 0:
...
def main() -> int:
    ...
    parser.add_argument("--commit", action="store_true", help="Git commit the baseline update")
