...
class TankPixelRenderer:
    ...
    def create_turret_overlay(
        ...
        if tank_type == TankType.SHERMAN_M4:
            ...
            schurzen_y = ty + 4
            pygame.draw.line(
                surface, tp["schurzen"], (tx - 1, schurzen_y), (tx - 1, schurzen_y + 5), 1
            )
            pygame.draw.line(
                surface, tp["schurzen"], (tx + tw, schurzen_y), (tx + tw, schurzen_y + 5), 1
            )
        elif tank_type == TankType.TIGER_I:
            tw, th = 16, 14
            tx = cx - tw // 2
        ...
        gun_length = (
            ...
            if tank_type == TankType.PANTHER_AUSFG
            else 14
        )
    ...
    def _draw_sherman_m4(surface, direction, turret_direction, state, frame, tp, cx, cy):
        ...
        track_h = hull_h + 2
        pygame.draw.rect(
            hull_temp, tp["track_color"], (hull_x - track_w, hull_y - 1, track_w, track_h)
        )
        pygame.draw.rect(
            hull_temp, tp["track_color"], (hull_x + hull_w, hull_y - 1, track_w, track_h)
        )

        for ty in range(hull_y - 1, hull_y + track_h, 2):
            pygame.draw.line(hull_temp, (55, 55, 55), (hull_x - track_w, ty), (hull_x, ty), 1)
            pygame.draw.line(
                hull_temp, (55, 55, 55), (hull_x + hull_w, ty), (hull_x + hull_w + track_w, ty), 1
            )
        ...
        pygame.draw.line(
            hull_temp, seam, (hull_x + 1, hull_y + 7), (hull_x + hull_w - 1, hull_y + 7), 1
        )
        pygame.draw.line(
            hull_temp,
            seam,
        ...
        pygame.draw.rect(
            hull_temp,
            tp["body_dark"],
            (hull_x + 2, hull_y + hull_h - engine_h, hull_w - 4, engine_h),
        )
    ...
    def _draw_panther_ausfg(surface, direction, turret_direction, state, frame, tp, cx, cy):
        ...
        for ty in range(hull_y - 1, hull_y + track_h, 2):
            ...
            pygame.draw.line(
                hull_temp, (48, 48, 48), (hull_x + hull_w, ty), (hull_x + hull_w + track_w, ty), 1
            )
        ...
        if camo_rng.random() > 0.5:
            for stripe_y in range(hull_y + 4, hull_y + hull_h - 4, 3):
                ...
                pygame.draw.line(
                    hull_temp,
                    tp["camo_red_brown"],
                    (stripe_start, stripe_y),
                    (stripe_start + stripe_len, stripe_y),
                    1,
                )

        panther_seam = tuple(max(0, c - 18) for c in tp["body_dark"])
        pygame.draw.line(
            hull_temp, panther_seam, (hull_x + 1, hull_y + 8), (hull_x + hull_w - 1, hull_y + 8), 1
        )
        pygame.draw.line(
            hull_temp,
            panther_seam,
            (hull_x + 1, hull_y + hull_h - 8),
            (hull_x + hull_w - 1, hull_y + hull_h - 8),
            1,
        )
        ...
        for vy in range(hull_y + hull_h - engine_h + 1, hull_y + hull_h - 1, 2):
            pygame.draw.line(
                hull_temp, panther_seam, (hull_x + 4, vy), (hull_x + hull_w - 4, vy), 1
            )
        ...
        TankPixelRenderer._draw_iron_cross(
            hull_temp, tcx, hull_y + hull_h // 2 + 2, tp["iron_cross"]
        )
        ...
        schurzen_y = ty + 4
        pygame.draw.line(
            turret_temp, tp["schurzen"], (tx - 1, schurzen_y), (tx - 1, schurzen_y + 5), 1
        )
        pygame.draw.line(
            turret_temp, tp["schurzen"], (tx + tw, schurzen_y), (tx + tw, schurzen_y + 5), 1
        )
    ...
    def _draw_tiger_i(surface, direction, turret_direction, state, frame, tp, cx, cy):
        ...
        for ty in range(hull_y - 1, hull_y + track_h, 2):
            ...
            pygame.draw.line(
                hull_temp, (42, 42, 42), (hull_x + hull_w, ty), (hull_x + hull_w + track_w, ty), 1
            )
        ...
        pygame.draw.line(
            hull_temp, tiger_seam, (hull_x + 1, hull_y + 8), (hull_x + hull_w - 1, hull_y + 8), 1
        )
        pygame.draw.line(
            hull_temp,
            tiger_seam,
        ...
        pygame.draw.line(
            hull_temp, tiger_seam, (tcx - 4, hull_y + 1), (tcx - 4, hull_y + hull_h - 1), 1
        )
        pygame.draw.line(
            hull_temp, tiger_seam, (tcx + 4, hull_y + 1), (tcx + 4, hull_y + hull_h - 1), 1
        )
        ...
        if state == "move":
            ...
            for i in range(5):
                ...
                dust_y = cy + int(dvy * dust_dist)
                pygame.draw.circle(
                    surface, (139, 119, 66), (dust_x, dust_y), 2 if i % 2 == 0 else 1
                )
