...
class EnhancedPixelArtist:
    ...
    def create_historical_infantry(
        ...
        pygame.draw.ellipse(surface, shadow_color, 
                          (center_x - 6, center_y + 10, 12, 4))
        
        # 绘制身体（考虑方向）
        if angle < 90 or angle > 270:  # 朝北方向
            # 腿部
            pygame.draw.rect(surface, uniform_color,
                           (center_x - 3, center_y + 2, 2, 8 + leg_offset))
            pygame.draw.rect(surface, uniform_color,
                           (center_x + 1, center_y + 2, 2, 8 - leg_offset))
            # 身体
            pygame.draw.rect(surface, uniform_color,
                           (center_x - 3, center_y - 4, 6, 8))
            # 装备（背包）
            pygame.draw.rect(surface, equipment_color,
                           (center_x - 2, center_y - 3, 4, 5))
            # 钢盔
            pygame.draw.circle(surface, helmet_color,
                             (center_x, center_y - 8), 4)
            # 武器（步枪）
            pygame.draw.line(surface, equipment_color,
                           (center_x + 3, center_y - 2),
                           (center_x + 5, center_y + 6), 2)
        else:  # 朝南方向
            # 腿部
            pygame.draw.rect(surface, uniform_color,
                           (center_x - 3, center_y + 2, 2, 8 + leg_offset))
        ...
        # 添加高光
        pygame.draw.circle(surface, CC2_COLORS["metal_highlight"],
                         (center_x - 1, center_y - 9), 1)
    ...
    def create_historical_tank(
        ...
        if 45 <= angle <= 135:  # 侧面视角 (东)
            ...
            pygame.draw.rect(surface, track_color,
                           (center_x - 11, center_y + 3, 22, 4))
            pygame.draw.rect(surface, track_color,
                           (center_x - 11, center_y - 5, 22, 4))
            
            # 车体
            pygame.draw.rect(surface, base_color,
                           (center_x - 10, center_y - 4, 20, 7))
            
            # 炮塔
            turret_x = center_x - 2
            pygame.draw.rect(surface, base_color,
                           (turret_x, center_y - 8, 8, 6))
            
            # 主炮
            gun_length = 15 if tank_name == "Sherman" else 18
            pygame.draw.rect(surface, (60, 60, 55),
                           (turret_x + 7, center_y - 6, gun_length, 2))
            
            # 高光
            pygame.draw.line(surface, CC2_COLORS["metal_highlight"],
                           (center_x - 9, center_y - 3),
                           (center_x + 9, center_y - 3), 1)
            ...
            gun_length = 15 if tank_name == "Sherman" else 18
            pygame.draw.rect(surface, (60, 60, 55),
                           (turret_x - gun_length, center_y - 6, gun_length, 2))
            ...
            track_color = CC2_COLORS["track_dark"]
            pygame.draw.rect(surface, track_color,
                           (center_x - 10, center_y, 4, 10))
            pygame.draw.rect(surface, track_color,
                           (center_x + 6, center_y, 4, 10))
            
            # 车体
            pygame.draw.rect(surface, base_color,
                           (center_x - 9, center_y - 2, 18, 12))
            
            # 炮塔
            pygame.draw.rect(surface, base_color,
                           (center_x - 5, center_y - 6, 10, 8))
            
            # 主炮
            if angle < 90 or angle > 270:  # 朝北
                pygame.draw.rect(surface, (60, 60, 55),
                               (center_x - 1, center_y - 14, 2, 8))
            else:  # 朝南
                pygame.draw.rect(surface, (60, 60, 55),
                               (center_x - 1, center_y + 2, 2, 8))
            
            # 高光
            pygame.draw.rect(surface, CC2_COLORS["metal_highlight"],
                           (center_x - 4, center_y - 5, 1, 6))
        
        return surface
    
    @staticmethod
    def create_historical_halftrack(
        faction: str,
        direction: str,
        size: int = 32
    ) -> Surface:
        ...
        # 阴影
        pygame.draw.ellipse(surface, CC2_COLORS["shadow"],
                          (center_x - 8, center_y + 7, 16, 5))
        
        # 根据方向绘制
        if 45 <= angle <= 135 or 225 <= angle <= 315:  # 侧面
            # 履带
            pygame.draw.rect(surface, CC2_COLORS["track_dark"],
                           (center_x - 8, center_y + 2, 16, 4))
            
            # 车体
            pygame.draw.rect(surface, base_color,
                           (center_x - 7, center_y - 4, 14, 8))
            
            # 驾驶舱
            pygame.draw.rect(surface, base_color,
                           (center_x - 6, center_y - 7, 6, 4))
            
            # 车轮（前部）
            pygame.draw.circle(surface, (40, 40, 35),
                             (center_x - 5, center_y + 4), 2)
            pygame.draw.circle(surface, (40, 40, 35),
                             (center_x + 5, center_y + 4), 2)
            ...
            pygame.draw.line(surface, CC2_COLORS["metal_highlight"],
                           ...
                           (center_x + 6, center_y - 3), 1)
        else:  # 正面/背面
            # 履带
            pygame.draw.rect(surface, CC2_COLORS["track_dark"],
                           (center_x - 8, center_y + 1, 3, 8))
            pygame.draw.rect(surface, CC2_COLORS["track_dark"],
                           (center_x + 5, center_y + 1, 3, 8))
            
            # 车体
            pygame.draw.rect(surface, base_color,
                           (center_x - 7, center_y - 2, 14, 10))
            
            # 驾驶舱
            pygame.draw.rect(surface, base_color,
                           (center_x - 4, center_y - 6, 8, 5))
            
            # 挡风玻璃
            pygame.draw.rect(surface, (80, 100, 120),
                           (center_x - 2, center_y - 5, 4, 2))
...
def create_historical_sprite(
    ...
    if "infantry" in unit_type_lower or "squad" in unit_type_lower:
        ...
        return EnhancedPixelArtist.create_historical_tank(
            faction, "medium", direction, None, size
        )
    elif "halftrack" in unit_type_lower:
        return EnhancedPixelArtist.create_historical_halftrack(
            faction, direction, size
        )
    else:
        # 默认返回步兵
        return EnhancedPixelArtist.create_historical_infantry(
