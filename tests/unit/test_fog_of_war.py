...
class TestFowBasic:
    ...
    def test_clear_current_visibility(self):
        ...
        assert len(cleared) >= 1, (
            f"Clearing visibility should return at least 1 tile, got {len(cleared)}"
        )
        for c in cleared:
            assert fow.is_explored(c)
            assert not fow.is_visible(c)
...
class TestFowRay:
    def test_36_rays_full_circle(self):
        ...
        assert len(visible) >= 21, (
            f"36-ray full-circle vision should reveal at least 21 tiles (center + ~20 in radius 6), got {len(visible)}"
        )
