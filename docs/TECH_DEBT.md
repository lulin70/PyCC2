...
| 🆕 **D15 7维度评估新增 (TD-073~078)** | **6** | 🔴 P0×3 + 🟡 P1×1 + 🟢 P2×2 | ✅ **v0.7.0: TD-073/074/075/076a/076b/076c/076d/078 全部解决 (7/8)** — 3 ghost 删除 + 1 type:ignore 修复 + context_menu 删除 + 3 半集成模块激活 (weapon_jam/surrender/campaign_persistence) + 1 DDD 分层修复 (deployment_factory 迁移); ✅ **v0.7.1: TD-077 19 孤立原型分类完成 (DELETE 4 + ARCHIVE 10 + ORPHAN 8)** (2026-07-17) |
| **合计（活跃）** | **0** | — | **64/64 历史 + 8 新发现 = 72 总计，0 活跃 (全部解决)** |
...
### 🟢 TD-COV-BUG: 覆盖率提升过程中发现的源码 bug (9 项，全部已修复)
...
9. **lighting_effects.spawn_dynamic_light / render_dynamic_lights L408** — `effect["position"]` 是 Vec2 对象（来自 `unit.position.pixel_position`），但 `render_dynamic_lights` 用 `pos[0]` 下标访问，Vec2 不支持下标。修复 #1/#2 阵营硬编码后，axis 玩家阵营正确识别触发 AI（allied）攻击，暴露此 bug

- **修复方案**: 见 [TD-COV-BUG_FIX_PLAN.md](TD-COV-BUG_FIX_PLAN.md)
- **验证**: 6178 passed / 0 failed / 21 skipped (not slow, no:randomly)，ruff 0 errors
- **状态**: 🟢 全部已修复（v0.6.7）
...
### ~~🔴 TD-073: spritesheet_parser.py ghost 模块 (P0)~~ ✅ RESOLVED (v0.6.11, 2026-07-17)
...
- **状态**: ✅ RESOLVED (v0.6.11, 2026-07-17) — 源文件 508 行删除 + test_phase_b_final_sprint.py TestSpritesheetParser 类 (8 测试方法) 删除 + docstring 移除 SpritesheetParser 引用; 7-Role 共识: 删除 (PLANNED + 0 src 引用 + 未来需求重新设计更优)
- **验证**: 6486 passed / 2 skipped / 16 deselected, ruff 0, mypy 0, 零回归

### ~~🔴 TD-074: operation_timeline.py ghost 模块 (P0)~~ ✅ RESOLVED (v0.6.11, 2026-07-17)
...
- **状态**: ✅ RESOLVED (v0.6.11, 2026-07-17) — 源文件 151 行删除 + test_strategic_ui.py 6 TestTimeline* 类 + 4 TestCampaignStateToTimelineIntegration 测试删除 (共 26 测试) + docstring/import 移除; 7-Role 共识: 删除 (PLANNED + 0 src 引用 + 历史数据可在未来重新设计时恢复)
- **验证**: 6486 passed / 2 skipped / 16 deselected, ruff 0, mypy 0, 零回归

### ~~🔴 TD-075: tactical_ai_types.py type:ignore[name-defined] 高风险 (P0)~~ ✅ RESOLVED (v0.6.11, 2026-07-17)
...
- **状态**: ✅ RESOLVED (v0.6.11, 2026-07-17) — L68 `intent: pycc2.domain.ai.tactic_intent.TacticIntent  # type: ignore[name-defined]  # noqa: F821` → `intent: TacticIntent`; 安全性: 文件已含 `from __future__ import annotations` (L13) + TYPE_CHECKING import of TacticIntent (L27); L185 已使用 `list[TacticIntent]` 同模式
- **验证**: mypy 0 errors (MYPYPATH=src mypy -p pycc2 --no-error-summary), ruff 0, 6486 passed 零回归

### ~~🟡 TD-076: 4 个半集成 ghost 候选验证 (P1)~~ ✅ RESOLVED (v0.7.0, 2026-07-17)
...
- **7-Role 共识结论** (2026-07-17 v0.6.11 + 2026-07-17 v0.7.0 规划):
  ...
  - **TD-076b surrender_system (403L)**: ✅ **INTEGRATED v0.7.0 Wave 2** — SurrenderAI 已在 `ai_service.py.__init__()` 注册到 TacticalOrchestrator; SurrenderSystem 实例化并暴露为 `ai_service.surrender_system` property
  - **TD-076c weapon_jam (252L)**: ✅ **INTEGRATED v0.7.0 Wave 1** — WeaponJamSystem 已在 `ai_service.py.__init__()` 实例化并暴露为 `ai_service.weapon_jam_system` property; `tick()` 中调用 `WeaponJamSystem.tick(unit)`; 修复了 `tick()` 的 off-by-one bug (clear_ticks=N 现在精确 N ticks 清除, 非 N+1)
  - **TD-076d campaign_persistence (373L)**: ✅ **INTEGRATED v0.7.0 Wave 3** — CampaignPersistenceManager 已在 `game_loop_assembler.py` 接线; `GameLoop.campaign_persistence` property 暴露; `GameLoop.save_campaign()`/`load_campaign()` 方法可用
- **状态**:
  ...
  - ✅ TD-076b/c/d RESOLVED (v0.7.0, 2026-07-17) — 3 个半集成模块全部接入游戏循环; 详见 docs/ROADMAP_v0.7.0.md
- **验证 (v0.7.0)**: 6509 passed / 2 skipped / 16 deselected, ruff 0, mypy 2 pre-existing errors (config.rendering_features, 与 v0.7.0 无关), 12 新增 E2E 测试 (test_v0_7_0_ghost_integration_e2e.py), 零回归

### ~~🟢 TD-077: 19 个孤立原型评估 (P2)~~ ✅ RESOLVED (v0.7.1, 2026-07-17) + ✅ INTEGRATE COMPLETE (v0.7.6, 2026-07-18)
...
- **状态**: ✅ RESOLVED (v0.7.1, 2026-07-17) — 19 个模块分类决策完成: DELETE 4 (strategic_map_view/strategic_map/aar_panel/weather_visual_effects — 被活跃模块取代) + ARCHIVE 10 (ai_config/casualty_system/ammo_type_system/combat_log/cc2_hud/enhanced_ui_renderer/enhanced_post_processing + 3 连带 hud_constants/hud_input/hud_renderer — 自闭合死代码群) + ORPHAN 标记 8 (cover_seek_ai/psychology_system/squad_group_manager/day_night_cycle/path_preview/range_indicator/vehicle_variant_generator/faction_variant_generator — pending v0.8.0+ INTEGRATE); 测试清理: 4 文件归档 (tests/_archive/) + 4 文件分离清理 (删除归档模块测试类保留活跃模块) + 1 文件删除 (TestWeatherVisualEffects 7 tests); 验证: 6157 collected / 6138 passed / 2 skipped, ruff 0, check_doc_consistency 11/11 PASS; 详见 docs/ROADMAP_v0.7.1.md
- **状态更新**: ✅ INTEGRATE COMPLETE (v0.7.6, 2026-07-18) — 8 个 ORPHAN 模块全部接入游戏循环:
  - **v0.7.5 (2 modules, 27 integration tests)**: `psychology_system` 接入 TacticalOrchestrator.tick() Phase 5 按士气/压制/疲劳过滤被拒命令 + `squad_group_manager` 接入 InputRouter Ctrl+1~9/1~9 + Minimap 边界框渲染
  - **v0.7.6 (6 modules, 75 integration tests)**: `path_preview` (8 tests) + `range_indicator` (10 tests) 接入 Minimap 移动路径预览与射程圈 + `cover_seek_ai` (9 tests) 接入 TacticalOrchestrator.tick() Phase 6 按压制状态驱动掩体寻找 + `day_night_cycle` (34 tests) 通过 GameTime 注入激活 v0.7.3 预留的音频/阴影/天气适配代码 + `vehicle_variant_generator` + `faction_variant_generator` (14 tests) 接入 unit_database 构建管线使模板 80→110
  - **验证**: 6311 collected / 6291 passed / 2 skipped / 18 deselected (59.16s), 503 E2E + 256 integration, ruff 0, 详见 docs/ROADMAP_v0.7.6.md

### ~~🟢 TD-078: deployment_manager.py 架构 borderline smell (P2)~~ ✅ RESOLVED (v0.7.0, 2026-07-17)
...
- **状态**: ✅ RESOLVED (v0.7.0, 2026-07-17) — 3 个纯函数 (build_force_pool_from_settings/generate_ai_deployment/build_default_roster) 从 `presentation/ui/deployment_factory.py` 迁移到 `services/deployment_factory.py`; domain 类型 (DeploymentUnit/UnitCategory/UNIT_TYPE_TO_CATEGORY/IMPASSABLE_TERRAINS) 迁移到 `domain/value_objects/deployment_types.py`; `presentation/ui/deployment_models.py` 改为 re-export domain 类型保持向后兼容; 旧 `presentation/ui/deployment_factory.py` 已删除; 4 个 importer 更新 (deployment_manager.py + deployment_ui.py + deployment_zone_builder.py + deployment_models.py); 8 架构守卫测试 + 200 deployment 测试全绿
