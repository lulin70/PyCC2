...
- **状态**: 🟡 进行中 — 单测前置补齐 (v0.4.3 batch 2 完成 10/19)
- **触发拆分的时机**:
...
- **将来拆分的推荐顺序**:
  1. 先补 19 个无测试 handler 的单测（锁定行为）— **v0.4.3 进行中: 10/19 完成 (batch 1-2)**
     ...
     - ⏳ Batch 3-4 (待推进): RALLY_NCO / SCAVENGE_AMMO / HEAL_WOUNDED / DIG_TRENCH / LAY_MINE / CLEAR_BUILDING / MOUNT_TANK / DISMOUNT_TANK / ASSAULT_FORTIFIED
...
- **本次已完成**: deployment_ui.py 拆分（commit 88fe1b9），tactic_executor.py 评估并记录；v0.4.3 batch 1-2 单测补齐 10/19 (35 tests)
