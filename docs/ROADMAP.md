...
**v0.7.7 | July 18, 2026 | Based on DevSquad 7-Role Analysis**

> **Current Version**: v0.7.7 | **Tests**: ~6291 (all passing) | **CC2 Fidelity**: ~75% (Visual ~70% / Mechanics ~80%) ⚠️ v0.5.0 P0 PixVoxel 接入 + v0.5.1 P2 isometric 清理 + v0.5.2 P1 调色板修正 + v0.5.3 P1 纹理调优 + v0.6.0 P3-1 LOS 烟雾天气 + v0.6.1 P3-2 窗户射击弧 + v0.6.2 P3-3 散兵坑战壕 + v0.6.3 P3-4 AI 侦察行为 + v0.6.4 P3-5 AI 心理模型 + v0.6.5 P3-6 AI 补给线意识 + v0.6.6 P0-P1 修复 + v0.6.7 TD-COV-BUG 9项源码bug修复 + v0.6.8 R3 评估修复 (幽灵功能接入 + flaky 测试修复 + CI 安全增强) + v0.6.10 覆盖率提升+CI增强 (文档校准 + 脚本归档 + 323 新测试 + radon 复杂度门禁) + v0.6.11 ghost 模块清理 (TD-073/074/075/076a: 3 删除 + 1 type:ignore 修复) + v0.7.0 TD-076b/c/d 接入 + TD-077 三步走 (DELETE 4 + ARCHIVE 10 + ORPHAN 8) + v0.7.2 INTEGRATE 前置准备 (文档同步 + flaky benchmark 修复 + ORPHAN smoke tests) + v0.7.3 day_night_cycle 接口兼容性修复 (time_of_day 重命名 + 新增 float 返回) + v0.7.4 INTEGRATE 前置准备 (squad_group_manager 测试增强 5→35 tests + psychology_system/squad_group_manager INTEGRATE 设计文档) + v0.7.5 INTEGRATE 完成 (psychology_system 接入 TacticalOrchestrator.tick() Phase 5 过滤被拒绝命令 + squad_group_manager 接入 InputRouter Ctrl+1~9 创建/1~9 选择 + Minimap 边界框渲染 + 27 集成测试) + v0.7.6 TD-077 ORPHAN 全清 (path_preview + range_indicator 接入 Minimap + cover_seek_ai 接入 Phase 6 + day_night_cycle 激活 GameTime + vehicle/faction_variant_generator 接入 unit_database 80→110, 75 集成测试) + v0.7.7 P2 技术债清理 (4 个 F 级函数重构: generate_road cc=115→6 + handle_player_command cc=73→4 + generate_orders cc=68→19 + draw_units cc=63→14, E+ violations 22→18; radon baseline 配置化 CI 硬编码 → pyproject.toml [tool.radon] 单点维护, 6291 测试零回归)
...
| **v0.7.2** | **INTEGRATE Prep** | **M3** | **~75%** | **~6156** | **INTEGRATE 前置准备: ROADMAP.md 文档同步 + flaky benchmark threshold 修复 (100ms→130ms) + 3 ORPHAN 模块 smoke tests (squad_group_manager/path_preview/range_indicator) + variant_generator weapon_id 验证** | ✅ Completed |
| v0.7.3 | Interface Fix | M3 | ~75% | ~6156 | day_night_cycle 接口兼容性修复: time_of_day 重命名为 time_phase (enum) + 新增 time_of_day 返回 float (0.0-24.0) + game_loop_updating 调用方适配 | ✅ Completed |
| **v0.7.4** | **INTEGRATE Prep** | **M3** | **~75%** | **~6189** | **INTEGRATE 前置准备: squad_group_manager 测试增强 5→35 tests (6 维度覆盖 Happy/Error/Boundary/Performance/Configuration/Integration) + psychology_system/squad_group_manager INTEGRATE 设计文档** | ✅ Completed |
| **v0.7.5** | **INTEGRATE** | **M3** | **~75%** | **~6216** | **INTEGRATE 完成: psychology_system 接入 TacticalOrchestrator.tick() Phase 5 过滤被拒绝命令 (11 集成测试) + squad_group_manager 接入 InputRouter Ctrl+1~9 创建/1~9 选择 + Minimap 边界框渲染 (16 集成测试)** | ✅ **Current** |
...
**Created**: 2026-05-19
**Updated**: 2026-07-18
**Status**: Beta Candidate — P0-P3 全部完成; v0.6.10 覆盖率提升+CI增强; v0.6.11 ghost 模块清理; v0.7.0 TD-076b/c/d 接入; v0.7.1 TD-077 三步走 (DELETE 4 + ARCHIVE 10 + ORPHAN 8); v0.7.2 INTEGRATE 前置准备 (文档同步 + flaky benchmark 修复 + ORPHAN smoke tests); v0.7.3 day_night_cycle 接口兼容性修复 (time_of_day 重命名为 time_phase + 新增 time_of_day 返回 float 0.0-24.0 + game_loop_updating 调用方适配); v0.7.4 INTEGRATE 前置准备 (squad_group_manager 测试增强 5→35 tests + psychology_system/squad_group_manager INTEGRATE 设计文档); v0.7.5 INTEGRATE 完成 (psychology_system 接入 TacticalOrchestrator.tick() Phase 5 + squad_group_manager 接入 InputRouter Ctrl+1~9/1~9 + Minimap 边界框渲染); v0.7.6 TD-077 ORPHAN 全清 (path_preview + range_indicator + cover_seek_ai + day_night_cycle + vehicle/faction_variant_generator 接入游戏循环, 75 集成测试, TD-077 全部 8 个 ORPHAN 模块接入完成)
**Next Review**: v0.8.0+ Beta 候选收尾 (视觉/音频打磨 + 平衡性调整 + 难度曲线优化) 或 M6 内容增强 (新增地图/单位/战役)
**Related Documents**: [GAP_ANALYSIS.md](docs/GAP_ANALYSIS.md) | [TECH_DEBT.md](docs/TECH_DEBT.md) | [VISUAL_FIDELITY_IMPROVEMENT_PLAN.md](VISUAL_FIDELITY_IMPROVEMENT_PLAN.md) | [ROADMAP_v0.7.4.md](docs/ROADMAP_v0.7.4.md) | [ROADMAP_v0.7.5.md](docs/ROADMAP_v0.7.5.md) | [ROADMAP_v0.7.6.md](docs/ROADMAP_v0.7.6.md)
