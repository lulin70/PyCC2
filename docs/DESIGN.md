# PyCC2 技术设计文档 (DESIGN) **v0.7.2**

> **项目**: Close Combat 2 Python 重写 | **阶段**: M5 — Beta Candidate (质量可持续)
> **输入**: P2 ADD v1.0 (架构设计) + P1 PRD v1.0 (需求规格) + M1/M2/M3/M4/M5 修复与新增方案
> **文档版本**: v0.7.2 | **日期**: 2026-07-17
> **架构演进**: v0.4.x DDD四层 → v0.5.x GameLoopAssembler组合根 + Domain瘦身38.5% → v0.6.x 幽灵功能接入 + CI radon门禁 + 323新测试 → v0.6.11 ghost 模块清理 (TD-073/074/075/076a) → v0.7.0 半集成模块接入 (TD-076b/c/d + TD-078 架构修复) → v0.7.1 ghost 模块分类清理 (TD-077: DELETE 4 + ARCHIVE 10 + ORPHAN 8) → v0.7.2 INTEGRATE 前置准备 (文档同步 + flaky benchmark 修复 + ORPHAN smoke tests)
...
│  └─────────────────┘  └─────────────────┘  └─────────────┘ │
│                                                             │
│  3 Sectors / 7 Operations / 29 Battles / 63 Maps           │
└─────────────────────────────────────────────────────────────┘
```
...
## 第14部分：v0.3.31-v0.4.0 架构更新 (M3 新增)
...
| 指标 | v0.3.0 | v0.4.0 | v0.6.10 | 变化趋势 |
|------|--------|---------|---------|----------|
| 测试数 | 3372 | ~3985 | **~6536** | +3164 (v0.6.10 覆盖率提升) |
| CC2还原度 | ~95% | ~88% | **~75%** | v0.4.16 诚实修正 (原88%高估) |
| 层违规 | ~41 | ~25 | ~25 | -39% (v0.3.x 修复后稳定) |
| EnhancedRenderer行数 | 1389 | 943 | 943 | -32% (v0.3.37 拆分后稳定) |
| 裸print() | 200+ | ~1 | ~1 | 99.3%清理 (v0.3.x 完成后稳定) |
| E2E测试 | 17阶段/dummy | 38阶段/real SDL | 38阶段/real SDL + 490 E2E | v0.5.0 扩展 |
| 抽出渲染模块 | 9 | 22 | 22 | +13 (v0.3.x 完成后稳定) |
| God Classes (>1000行) | 8 | 4 | **0** (P2-2 取消) | 4候选全部 SRP 分析判定 NOT God Class |
| 源码模块数 | — | ~390 | **388** | v0.5.1 isometric 清理后减少 |
| 测试文件数 | — | ~180 | **210** | v0.6.10 覆盖率提升新增 |

> **注**: v0.6.10 列数据通过 `find src/pycc2 -name "*.py" | wc -l` (388) 和 `find tests -name "*.py" | wc -l` (210) 实测验证。God Class 评估基于 SRP 分析（而非行数/方法数阈值），详见 project_memory Lessons Learned。
