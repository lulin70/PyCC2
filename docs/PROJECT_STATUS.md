...
| 测试文件数 | 166 个 `.py` 文件 | `find tests -name "*.py" \| wc -l` |
| 测试用例数 | 4471 collected / 4445 passed / 25 skipped | `pytest --collect-only` + 全量回归 |
| 覆盖率门禁 | pyproject.toml `fail_under=60`（目标）+ CI `--cov-fail-under=50`（暂调让 CI 绿） | `.github/workflows/ci.yml` |
| 实际覆盖率 | 54% (44167 stmts, 18435 missed，较 52.66% 提升 1.3%) | `pytest tests/unit/ --cov=src/pycc2 --cov-report=term` |
...
- [x] 全量回归测试通过
- [ ] CI 全绿（Lint 已修复；unit-tests 覆盖率 54% 已达暂调后门禁 50%，但距 pyproject.toml 目标 60% 仍有差距）
