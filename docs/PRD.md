# PyCC2 产品需求规格书 (PRD) v0.1.1 — 诚实评估版

## 一、项目概述

- **项目名称**: PyCC2 (Python Close Combat 2)
- **目标**: 用 Python 从零重写经典 RTT 游戏 Close Combat 2: A Bridge Too Far
- **平台**: macOS Apple M1 (ARM64)
- **技术栈**: Python 3.11+ / Pygame 2 (SDL2) / numpy (+可选 numba)
- **美术路线**: 几何原型 → 像素画渐进
...
**最后更新**: 2026-05-23
**作者**: PyCC2 项目组
**状态**: ⚠️ M1紧急修复进行中 — P0-1/P0-2/P0-3/P0-4已修复
**当前版本**: v0.1.1
**测试总数**: 2767 tests (1 failed)
**CC2还原度**: ~45%（诚实评估，原声称~97%严重高估）
**下一步**: 完成M1剩余任务（冒烟测试+set_mode扩展）
...
**当前还原度**: ~45%（诚实评估，详见下方"诚实还原度评估"章节）

---

## 一点五、当前P0问题（致命Bug — 修复进展）

> **原5个P0 Bug中3个已修复，2个待处理。**

| P0 ID | Bug描述 | 影响 | 根因 | 状态 |
|-------|---------|------|------|------|
| ~~P0-001~~ | ~~Unit缺少display_name属性~~ | ~~cc2_bottom_panel渲染崩溃~~ | ~~Unit类未定义display_name属性~~ | ✅ 已修复 |
| ~~P0-002~~ | ~~HealthComponent/MoraleComponent属性名不匹配~~ | ~~战斗结算崩溃~~ | ~~组件属性名与调用方期望不一致~~ | ✅ 已修复（添加property别名） |
| ~~P0-003~~ | ~~AttackNearestAI/MoveToObjectiveAI导入失败~~ | ~~AI系统完全瘫痪~~ | ~~AI行为类文件路径或类名与import不匹配~~ | ✅ 已修复 |
| P0-004 | set_mode()不支持fast/sneak参数 | 移动模式命令不可操作 | Unit.set_mode()仅接受字符串枚举 | ❌ 未解决 |
| P0-005 | 旧UI组件文件未清理 | 渲染冲突和ImportError | 旧版UI组件文件残留 | ❌ 未解决 |

**影响评估**: 原有5个P0 Bug中3个已修复，游戏基本可启动和运行。剩余2个P0 Bug影响命令系统和UI稳定性。

---

## 一点六、诚实还原度评估

> **基于DevSquad批判性Review的真实评估，不美化。**

| 维度 | v2.5声称值 | 诚实评估值 | 差距原因 |
|------|-----------|-----------|---------|
| **地图库** | 95% | **50%** | 28张地图JSON存在，渲染管线待完善 |
| **战役结构** | 90% | **30%** | 四层架构代码存在，AI基础已修复，高级AI待完善 |
| **武器系统** | 100% | **70%** | 69种武器数据定义完整，战斗结算已修复，待集成验证 |
| **单位多样性** | 95% | **40%** | 80种单位模板存在，渲染管线断裂 |
| **AI战术** | 85% | **15%** | 53+ behaviors代码存在，基础AI已修复，高级AI待完善 |
| **部署阶段** | 90% | **20%** | DeploymentUI代码存在，渲染待完善 |
| **视觉质量** | 120% | **25%** | EnhancedRenderer代码存在，旧UI残留冲突（P0-005），实际渲染不稳定 |
| **战斗机制** | 140% | **20%** | 压制/士气/隐蔽代码存在，属性别名已修复，待集成验证 |
| **游戏设置** | 110% | **60%** | 5预设+独立等级代码完整，这部分确实可用 |
| **音频** | 35% | **10%** | 程序化音效代码存在，但stereo预生成失败（TD-030），实际无声 |
| **综合还原度** | ~97% | **~45%** | 代码量庞大但集成断裂，核心路径全部不通 |

**核心问题**: 项目有大量"已实现"的代码模块，但模块间的接口契约未对齐，导致端到端流程完全断裂。2767个单元测试中绝大多数测试的是孤立模块，而非集成路径。1个失败的测试暴露了集成断裂点。M1修复已解决3个P0 Bug（display_name/属性别名/AI导入），剩余2个P0待修复。

---

## 二、User Stories (33 条)

> **标注说明**: ❌ = 当前不可用（因P0 Bug阻塞），⚠️ = 部分可用（代码存在但集成有问题），✅ = 可用

### 2.1 核心战斗系统 (US-COMBAT-001 ~ 005)

**US-COMBAT-001**: 作为玩家，我希望命令单位移动到指定位置，以便调整战术部署。 ⚠️
...
**US-MAP-001**: 作为玩家，我希望地图由不同类型的地形瓦片组成，以便产生丰富的战术变化。 ⚠️
- AC1: 至少支持12种地形类型(开阔地/道路/草地/林地/建筑/水域/树篱/墙等)
- AC2: 每种地形有不同的移动代价
- AC3: 林地和建筑提供掩体
- AC4: 水域和实心建筑不可通行

**US-MAP-002**: 作为玩家，我希望地形影响视线(LOS)，以便利用地形进行伏击。 ⚠️
- AC1: 林地和建筑阻挡视线
- AC2: 开阔地视线无遮挡
- AC3: 地形高度差影响视线(未来)
- AC4: 可以查看任意tile的LOS状态(debug)

**US-MAP-003**: 作为玩家，我希望加载自定义地图文件，以便扩展游戏内容。 ⚠️
- AC1: 地图数据存储为JSON格式
...
**US-UI-004**: 作为玩家，我希望顶部有状态栏显示任务信息，以便了解当前进度。 ⚠️
- AC1: 左侧显示任务名称
- AC2: 中间显示游戏时间和日期
- AC3: 右侧显示暂停/菜单按钮
- AC4: 暂停时游戏时间冻结
...
**US-CAMP-002**: 作为玩家，我希望可以保存和读取游戏存档，以便随时中断和恢复。 ⚠️
- AC1: 存档保存完整运行时状态
- AC2: 存档文件防篡改(HMAC签名)
- AC3: 读档时验证完整性
- AC4: 支持多个存档槽位
...
**US-FOW-001**: 作为玩家，我希望战争迷雾隐藏敌方位置，以便实现侦察战术。 ⚠️
- AC1: 仅显示己方视野内的敌方单位
...
### 系统还原度总览 (v0.1.1 诚实评估 — M1修复进展)

| 维度 | CC2原版 (1997) | PyCC2当前 | v2.5声称值 | 诚实评估值 | 评级 |
|------|---------------|----------|-----------|-----------|------|
| **地图库** | 25-30张历史地图(Arnhem 9张) | 28张JSON存在 | 95% | **50%** | 🟡 数据在但不可用 |
| **战役结构** | 四层架构(Grand Campaign→Sector→Operation→Battle) | 四层架构代码存在 | 90% | **30%** | 🔴 AI瘫痪无法运行 |
| **武器系统** | ~50种武器(含车辆武器+固定火炮) | 69种武器数据定义 | 100% | **70%** | 🟡 数据完整但结算崩溃 |
| **单位多样性** | 130+单位(含载具变体+固定武器) | 80种单位模板 | 95% | **40%** | 🟡 渲染管线断裂 |
| **AI战术** | 成熟行为树(侧翼/压制/步坦协同/炸桥) | 53+ behaviors代码存在 | 85% | **15%** | 🟡 基础AI已修复，高级AI待完善 |
| **部署阶段** | 完整拖放部署(3区域+限制) | DeploymentUI代码存在 | 90% | **20%** | 🔴 渲染崩溃 |
| **视觉质量** | 1997像素艺术 | EnhancedRenderer代码存在 | 120% | **25%** | 🔴 旧UI残留冲突 |
| **战斗机制** | 基础压制+士气 | 6级压制+隐蔽代码存在 | 140% | **20%** | 🔴 属性名不匹配 |
| **游戏设置** | 基础难度选择 | 5预设+独立等级 | 110% | **60%** | 🟡 唯一基本可用部分 |
| **音频** | 完整音效+语音+背景音乐 | 程序化音效代码存在 | 35% | **10%** | 🔴 stereo预生成失败 |
| **综合还原度** | — | — | ~97% | **~45%** | 🔴 Pre-MVP |

### v0.1.1 关键差距（详见 [ISSUES.md](ISSUES.md)）

1. **P0致命Bug** — 原5个P0 Bug中3个已修复（display_name/属性别名/AI导入），剩余2个待修复（set_mode参数/旧UI清理）
2. **集成断裂** — 各模块代码存在但接口契约未对齐，端到端流程大部分不通
3. **音频** — 程序化音效仅23种且stereo预生成失败，缺少真实音效/语音命令/BGM
4. **AI战术** — 基础AI已修复，高级AI行为待实现
5. **技术债** — 2个P0 + 7个P1 + 9个P2 未解决（详见 [TECH_DEBT.md](TECH_DEBT.md)）
...
| P8 | 地图与战役补全 | 55%→70% | 9+新地图/四层战役/补给线/部署UI | ⚠️ 代码存在但集成断裂 |
| P9 | AI战术提升+新游戏设置 | 70%→80% | P0 AI行为(7项)/5预设游戏设置/战役完成 | ⚠️ 代码存在但AI导入失败 |
...
| **P8** | 地图与战役补全 | CC2还原度 55%→70% | 9+新地图/四层战役/补给线/部署UI | ⚠️ 代码存在但集成断裂 |
| **P9** | AI战术提升+新游戏设置 | CC2还原度 70%→80% | P0 AI行为(7项)/5预设游戏设置/战役完成 | ⚠️ 代码存在但AI导入失败 |
...
## 十一、CC2 Fidelity Update (v1.8 进展总结)
...
**当前版本**: v0.1.1
**测试总数**: 2767 tests (1 failed)
**下一步**: 完成M1剩余任务（冒烟测试+set_mode扩展+旧UI清理）
